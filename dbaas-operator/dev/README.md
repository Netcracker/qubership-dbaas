...
The credentials are read from `dbaas-security-configuration-secret` (`users.json`), which in
production is generated by the aggregator's Helm chart; here the aggregator-mock manifest ships a
static one (the mock does not validate the password).
To exercise the **M2M** path (projected SA Bearer token) instead:
