...
spec:
  ...
  classifier:
    ...
    scope: service                 # required; must be "service" or "tenant" (CRD enum)
...
| `scope` | Yes | Must be `service` or `tenant` — a CRD enum; admission rejects any other value. The controllers re-check it as a backstop for objects admitted before the enum, failing with `InvalidConfiguration`. |
...
A base delay of zero or less, or a maximum below the base, falls back to the defaults, so a misconfigured flag cannot
