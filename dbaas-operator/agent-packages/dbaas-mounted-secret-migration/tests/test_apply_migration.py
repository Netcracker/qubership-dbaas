...
class MountedSecretApplyTest(unittest.TestCase):
    def test_apply_generates_resources_and_mount(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            # Registered as an optional string, so `helm lint` / `helm template`
            # still pass against the chart defaults; the deployer supplies the
            # real namespace.
            self.assertNotIn("DBAAS_OPERATOR_NAMESPACE", schema.get("required", []))
            self.assertEqual(
                schema["properties"]["DBAAS_OPERATOR_NAMESPACE"], {"type": "string"}
            )
    ...
    def test_classifier_service_identity_must_match_origin_service(self) -> None:
        # A templated deployment resolves the service name from a value; the
        # inventory identity and decisions.originService must agree so the two
        # installs of the chart keep one database each.
        with tempfile.TemporaryDirectory() as directory:
            tmp = Path(directory)
            repo = scaffold(tmp)
            broken = plan(repo)
            broken["inputs"]["datasources"][0]["classifier"]["microserviceName"] = "orders-v2"
            code, report = run_migration(repo, broken, "apply", tmp)
            self.assertEqual(code, 4)
            self.assertTrue(
    ...
    def test_superseded_declaration_that_still_templates_the_service_name_is_blocked(self) -> None:
        # The generated identity carries a literal microserviceName ("orders"), so
        # a legacy declaration that still templates it from {{$SERVICE_NAME}} is a
        # different, per-install identity. The runner must not collapse it and
        # delete the declaration -- it blocks and keeps the file.
        with tempfile.TemporaryDirectory() as directory:
            tmp = Path(directory)
            repo = scaffold(tmp)
            the_plan = plan(repo)
