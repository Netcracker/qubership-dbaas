...
no field for versioning or clone behavior, and the mounted-secret runner will not treat a
declaration that sets either as superseded -- it blocks instead of silently dropping the setting
(see `references/contracts.md`). Do not migrate that datasource automatically; flag it for a manual
follow-up instead of discarding the requirement.
