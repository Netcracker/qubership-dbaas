...
`InternalDatabase` has no field for versioning or initial-instantiation behavior in this migration's
contract, and `apply_migration.py` refuses to supersede a declaration that sets either -- do not
migrate that datasource automatically. Preserve structured settings as their corresponding YAML
