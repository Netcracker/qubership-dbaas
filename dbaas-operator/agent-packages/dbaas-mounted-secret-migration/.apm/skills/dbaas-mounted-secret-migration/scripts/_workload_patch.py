...
def patch_workloads(text: str, *, filename: str, targets: list[dict[str, Any]]) -> str:
    ...
    # Do every edit against LF text so a spliced-in block never picks up a stray
    # CRLF (or glues onto a final line that had no newline); restore the file's
    # original ending on the way out.
    uses_crlf = "\r\n" in text
    work = text.replace("\r\n", "\n") if uses_crlf else text

    _reject_helm_actions(work, filename)
    lines = work.splitlines(keepends=True)
    if lines and not lines[-1].endswith("\n"):
        lines[-1] += "\n"

    try:
        nodes = [node for node in yaml.compose_all(_sanitize(work, filename)) if node is not None]
