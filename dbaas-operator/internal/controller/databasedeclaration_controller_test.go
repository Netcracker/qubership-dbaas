...
var _ = Describe("DatabaseDeclaration Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind DatabaseDeclaration")
			err := k8sClient.Get(ctx, typeNamespacedName, databasedeclaration)
			if err != nil && errors.IsNotFound(err) {
				resource := &dbaasv1alpha1.DatabaseDeclaration{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance DatabaseDeclaration")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &DatabaseDeclarationReconciler{
				Client: k8sClient,
