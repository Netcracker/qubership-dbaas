...
for node in $nodes; do
  ...
  for log in dbaas-aggregator.log dbaas-postgres-adapter.log postgres-backup-daemon.log dbaas-operator.log go-test-app-service.log; do
    if docker exec "$node" sh -c "test -f /var/log/fluent-bit/${log}"; then
      ...
      docker exec "$node" sh -c "cat /var/log/fluent-bit/${log}" | \
        jq -Rr 'fromjson? | select(.kubernetes.pod_name and .log) | select(.kubernetes.pod_name|test("^(dbaas-aggregator|dbaas-postgres-adapter|postgres-backup-daemon|dbaas-operator|go-test-app-service)")) | [.kubernetes.pod_name, .log] | @tsv' | \
