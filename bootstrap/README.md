...
`helmfile/helmfile.yaml.gotmpl` provisions PostgreSQL, the DBaaS aggregator, the dbaas-operator, and the PostgreSQL
adapter with Kubernetes M2M enabled. Point it at local checkouts and set the image tags and generated credentials:

```bash
export AGGREGATOR_CHART_DIR=<qubership-dbaas checkout> AGGREGATOR_TAG=<aggregator image tag>
export OPERATOR_CHART_DIR=<qubership-dbaas checkout> OPERATOR_TAG=<operator image tag>
export PGSKIPPER_DIR=<pgskipper-operator checkout>
export REGION_DBAAS=database NODE_SELECTOR_DBAAS_KEY=region
export POSTGRES_PASSWORD=<password> DBAAS_CLUSTER_DBA_CREDENTIALS_PASSWORD=<password> DBAAS_TENANT_PASSWORD=<password> \
  DBAAS_DB_EDITOR_CREDENTIALS_PASSWORD=<password> DISCR_TOOL_USER_PASSWORD=<password> \
  BACKUP_DAEMON_DBAAS_ACCESS_PASSWORD=<password>
helmfile -f bootstrap/helmfile/helmfile.yaml.gotmpl sync
```

`DBAAS_NAMESPACE`, `PG_NAMESPACE`, `DBAAS_SERVICE_NAME`, and `AGGREGATOR_REPLICAS` default to `dbaas`, `postgres`,
`dbaas-aggregator`, and `1`. Select one release with a label, for example `-l component=aggregator`.
