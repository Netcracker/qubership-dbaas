...
// committed. A successful response carries only the caller's probe ID and status. Failures use the
// service's generic error response, so neither path exposes connection properties or credentials.
//
